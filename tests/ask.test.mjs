...
test('自由提问：规划→检索→分类，带会话键，缓存复用，计入每日次数',async()=>{
  ...
  const deps={
    ...
    extractComparison:async()=>({status:'complete',options:['高薪','成长'],sides:[],forks:[{label:'是应届生还是社招？',branches:[]}]})
  };
  await serve(createServer({...env,ZHIJING_LIVE_DAILY_LIMIT:'1'},deps),async base=>{
    ...
    assert.equal(data.comparison.forks[0].label,'是应届生还是社招？');
...
test('自由提问：查资料类 422 不检索；过短 400；跨站 403；未开放 503',async()=>{
  const deps={
    ...
    extractComparison:async()=>assert.fail('不应整理对比')
  };
...
test('自由提问：检索为空时提示换个说法，日志不含问题原文',async()=>{
  ...
  const deps={
    ...
    extractComparison:async()=>({status:'complete',options:[],sides:[],forks:[]})
  };
  await serve(createServer(env,deps),async base=>{
